...
|------|-------------|------|---------|:-----:|
| static\_ip\_name | Name of the Static IP resource to deploy for the Network LB | `any` | n/a | yes |
...
| cpu\_limit | CPU limit per Traefik pod | `string` | `""` | no |
...
| internal\_static\_ip | Whether the static IP to reserve is internal | `bool` | `false` | no |
| internal\_static\_ip\_subnetwork | The URL of the subnetwork in which to reserve the internal static address | `string` | `""` | no |
| kv\_store\_acme | Use the chart to configure Traefik to Store ACME certificates on Consul KV | `string` | `"false"` | no |
| label\_selector | Valid Kubernetes ingress label selector to watch | `string` | `""` | no |
| lb\_source\_range | List of CIDR allowed to access the Network Load balancer. This setting is enforced by GCP Network LB | `list` | <code><pre>[<br>  "0.0.0.0/0"<br>]<br></pre></code> | no |
| memory\_limit | Memory limit per Traefik pod | `string` | `"30Mi"` | no |
| memory\_request | Initial share of memory requested per Traefik pod | `string` | `""` | no |
| namespaces | List of Kubernetes namespaces to watch | `list` | <code><pre>[<br>  "default"<br>]<br></pre></code> | no |
...
| pod\_disruption\_budget | JSON string for pod disruption budget. See https://kubernetes.io/docs/tasks/run-application/configure-pdb/ | `string` | `""` | no |
...
| prometheus\_buckets | A list of response times (in seconds) - for each list element, Traefik will report all response times less than the element. | `list` | <code><pre>[<br>  0.005,<br>  0.01,<br>  0.025,<br>  0.05,<br>  0.075,<br>  0.1,<br>  0.25,<br>  0.5,<br>  0.75,<br>  1,<br>  2.5,<br>  5,<br>  7.5,<br>  10<br>]<br></pre></code> | no |
...
| whitelist\_source\_range | List of CIDRs allowed to access the Traefik Load balancer. This setting is enforced by Traefik. | `list` | <code><pre>[<br>  "0.0.0.0/0"<br>]<br></pre></code> | no |

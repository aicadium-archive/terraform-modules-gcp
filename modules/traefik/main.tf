...
locals {
  static_ip = var.internal_static_ip ? element(concat(google_compute_address.internal.*.address, [""]), 0) : element(concat(google_compute_address.external.*.address, [""]), 0)
...
EOF
  ...
  startup_arguments = slice(
    local.all_startup_args,
    local.consul_enabled ? 0 : 4,
    length(local.all_startup_args),
  )
...
locals {
  ...
  values = {
    ...
    service_annotations = jsonencode(
      merge(local.internal_service_annotation, var.service_annotations),
    )
